...
from .serializers import (
    ...
    IncidentStaffSerializer,
    IncidentMediaSerializer,
)


def _is_staff(request):
    """True only for an authenticated staff account (a coordinator)."""
...
class IncidentListView(generics.ListAPIView):
    ...
    def get_serializer_class(self):
        # Coordinators additionally see AI triage signals; the public never does.
        return IncidentStaffSerializer if _is_staff(self.request) else IncidentSerializer
...
def incident_detail(request, pk):
    ...
    serializer = IncidentStaffSerializer if _is_staff(request) else IncidentDetailSerializer
    return Response(serializer(incident).data)
...
def incident_track(request, pk):
    ...
    return Response({
        ...
        # note/actor withheld: internal triage reasoning + coordinator identity.
        "latest_log": {
...
def _is_safe_media_url(url: str) -> bool:
    ...
    These URLs are rendered as links and image sources on the PUBLIC incident
    page, so `javascript:`, `data:` and `file:` payloads would be a stored-XSS
    / local-file vector.
    """
...
def manage_media_urls(request, pk):
    """
    GET    → public: current media_urls list
    POST   → staff only: append a validated http(s) URL
    DELETE → staff only: remove a URL

    SECURITY: POST/DELETE previously performed NO authorisation check despite a
    docstring claiming otherwise, letting anyone attach arbitrary URLs to any
    incident's public page (stored XSS / phishing / unbounded growth).
    """
    incident = get_object_or_404(Incident, pk=pk)
    ...
    if request.method == "POST":
        if not _is_safe_media_url(url):
            return Response(
                {"error": "Only absolute http(s) URLs are allowed."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        if len(urls) >= MAX_MEDIA_URLS and url not in urls:
            return Response(
                {"error": f"Maximum {MAX_MEDIA_URLS} media URLs per incident."},
                status=status.HTTP_400_BAD_REQUEST,
            )
        if url not in urls:
            urls.append(url)
